  ...
  return (
    <div className="dial-core">
      ...
      <div className="core-timezone" ref={timezoneMenuRef}>
        ...
        {isOpen && <div className="timezone-menu" id="timezone-menu" role="listbox" aria-label={text.chooseTimezone}>{timezones.map((option, index) => <button ref={(element) => { optionRefs.current[index] = element; }} className={`timezone-option${option.id === timezone.id ? ' timezone-option--active' : ''}`} type="button" role="option" aria-selected={option.id === timezone.id} key={option.id} onClick={() => selectTimezone(option.id)}>{option.city}<small>{option.timeZone}</small></button>)}</div>}
      </div>
      <TimeReadout value={time} is24Hour={is24Hour} onToggle={onToggleTimeFormat} />
      <div className="date-navigator">
        <button className="date-nav-button" type="button" aria-label={text.previousDay} onClick={() => onShiftDate(-1)}>‹</button>
        <button className="time-readout-date" type="button" onClick={openDatePicker} aria-label={`${text.chooseDate}, currently ${dateLabel}`}>{dateLabel}</button>
        <button className="date-nav-button" type="button" aria-label={text.nextDay} onClick={() => onShiftDate(1)}>›</button>
        <input ref={dateInputRef} className="date-picker-input" type="date" value={dateValue} onChange={(event) => onDateChange(event.target.value)} aria-label={text.plannerDate} />
      </div>
      <button className="core-add-button" type="button" aria-label={text.addPlan} onClick={onAdd}><PlusIcon /></button>
    </div>
  );
}
